<script lang="ts">
  ...
  import type { Role, TableHeader, TablePagination, User } from "$lib/types";
  ...
  $: canWrite = hasAnyPermission(currentUser, ["roles.write"]);
...
<div class="w-full h-full px-4 grid gap-3">
  <div class="flex items-center justify-between">
    <Heading tag="h3" class="mb-2">Configuración de roles</Heading>
    {#if canWrite}
  ...
  >
    <TableBodyRow slot="row" let:row>
      ...
      <TableBodyCell
        >{row.allowedPermissions?.length ?? 0} permisos</TableBodyCell
      >
      <TableBodyCell>
        {#if canWrite}
